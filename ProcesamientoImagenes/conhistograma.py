...
def mostrar_histogramas(imagen_r, imagen_g, imagen_b):
    plt.figure(figsize=(6, 3))

    plt.subplot(131)
    plt.hist(imagen_r.histogram(), bins=256, color='red', alpha=0.7)
    plt.title('Histograma R')

    plt.subplot(132)
    plt.hist(imagen_g.histogram(), bins=256, color='green', alpha=0.7)
    plt.title('Histograma G')

    plt.subplot(133)
    plt.hist(imagen_b.histogram(), bins=256, color='blue', alpha=0.7)
    plt.title('Histograma B')

    # Limpiar el widget de Canvas
    canvas_histogramas.get_tk_widget().destroy()

    # Crear un nuevo widget de Canvas
    canvas_histogramas = FigureCanvasTkAgg(plt.gcf())
    canvas_histogramas.draw()

    # Ubicar el widget de Canvas en la ventana
    canvas_histogramas.get_tk_widget().pack(side=tk.BOTTOM, padx=10, pady=10)
...
canvas_histogramas = FigureCanvasTkAgg(plt.figure(figsize=(6, 3)))
canvas_histogramas.get_tk_widget().pack(side=tk.BOTTOM, padx=10, pady=10)
