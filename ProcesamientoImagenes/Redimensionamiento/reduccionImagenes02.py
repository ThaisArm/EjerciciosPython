...
def apply_filter(image, filter_size):
    ...
    filter_matrix = [[1] * filter_size for _ in range(filter_size)]

    # Creamos una nueva imagen filtrada con dimensiones reducidas
    reduced_width = width // filter_size
    reduced_height = height // filter_size
...
def apply_filter_and_show():
    filter_size = int(filter_entry.get())
    new_image = apply_filter(image, filter_size)
...
filter_label = tk.Label(buttons_frame, text="Dimensión del filtro:")
filter_label.pack(side="left", padx=5)

filter_entry = tk.Entry(buttons_frame)
